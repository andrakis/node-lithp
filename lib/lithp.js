...
exports.set_debug_flag = (V) => enable_debug = V;
		...
		switch(P.constructor) {
			...
			default:
				if(P.type == 'Atom')
					...
					return '[' + P.map(lithpInspectParser).join(', ') + ']';
		}
	var val = inspect(P);
	if(val.length > maxDebugLen
		&& P) // Technically not needed, since inspect(P) should not be
		      // less than "undefined". If maxDebugLen is smaller than
		      // this, crash may occur. But that's your own silly fault.
 ...
 */
Lithp.prototype.inspect = function(Args) {
	return Args.map(lithpInspectParser).join(', ');
};
...
Lithp.prototype.invoke_functioncall = function(chain, fndef, params) {
	...
	if(enable_debug) {
		...
		// debug is not on.
		debug_str = "FunctionCall (" + fndef.fn_name + (fndef.instance_id ? ':' + fndef.instance_id : '') + " " + this.inspect(params) + ")";
	}
	var arity = fndef.arity;
	var val;
	this.depth++;
	debug("+".repeat(this.depth * 2));
