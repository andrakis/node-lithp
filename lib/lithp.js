		...
		switch(P.constructor) {
			...
			default:
				if(P.type == 'Atom')
					...
					return '[' + P.map(V=>lithpInspectParser(P, Join)).join(Join) + ']';
		}
	var val = inspect(P);
...
Lithp.prototype.inspect = function(Args, Join) {
	Join = Join || ', ';
	return Args.map(V=>lithpInspectParser(V, Join)).join(Join);
};
...
Lithp.prototype.invoke_functioncall = function(chain, fndef, params) {
	...
	if(enable_debug) {
		...
		// debug is not on.
		debug_str = "FunctionCall (" + fndef.fn_name +
