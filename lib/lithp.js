...
function exitHandler(options, err) {
	if(global.lithp_print_times) {
		...
		for(var id in lithp_instances) {
			totalCalls += lithp_instances[id].functioncalls;
			info.push("lithp[" + id + "]: " + lithp_instances[id].functioncalls);
		}
		console.error(totalCalls + " function calls executed in " + total + "ms   across: [" + info.join(', ') + "]");
	}
