...
Lithp.prototype.builtin = function(name, params, body) {
	console.log("builtin, name: " + name + ", params:", params);
	if(name.indexOf("/") == -1) {
...
Lithp.prototype._do_functioncall = function(chain, i) {
	...
	if(enable_debug) {
		...
		// debug is not on.
		debug("FunctionCall " + fn_name + "(", params.map(P => {
			if(P)
				if(P.constructor === OpChain)
					return inspect(P.ops);
				else if(P.constructor === VariableReference)
					return inspect(P.ref);
			return inspect(P);
		}).join(', '), ")");
	}
...
Lithp.prototype.invoke_functioncall = function(chain, fndef, params) {
	// TODO: check params count, return curried function if not correct
